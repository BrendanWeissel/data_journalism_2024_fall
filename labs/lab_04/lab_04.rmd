...
**Note** - when I sorted the data in OpenRefine I sorted it in the "purpose" column so "purpose_clean" and "purpose" are flipped and my code reflects that.

**A1.**

```{r}
md_senate_expenses_cleaned2 <- read_csv("data/md_senate_expenses_cleaned2.csv")

hogan_spending <- md_senate_expenses_cleaned2 |> 
...
alsobrooks_spending <- md_senate_expenses_cleaned2 |> 
...
**A2.**The largest spending for Alsobrooks is Media Advertising and for Hogan it is ADS. This is exactly how it is listed in their filing but it is clear they each labeled their spending differently. Both of campaigns had salary/staffing expense as their second item. Alsobrooks spent a considerable amount of money on list aquisition whereas Hogan spent a lot on printing. Printing was 15th for Alsobrooks. 
...
**A3.** The Alsobrooks campaign spending on ads peaked in April while the Hogan campaign peaked in May. The spending slowly increased to both of those points and then dipped the following month. 

```{r}
hogan_ad_spending <- md_senate_expenses_cleaned2 |> 
  filter(candidate == "Hogan", purpose == "ADS") |> 
group_by(month = floor_date(date, "month")) |> 
  summarise(monthly_spending = (sum(amount)))

alsobrooks_ad_spending <- md_senate_expenses_cleaned2 |> 
  filter(candidate == "Alsobrooks", purpose == "Media Advertising") |> 
group_by(month = floor_date(date, "month")) |> 
  summarise(monthly_spending = (sum(amount)))
...
**A4.** Hogan has spent $481,109 with 18 transactions on staffing and Alsobrooks has spent $822,015 with 319 transactions. It would be interesting to see why Alsobrooks has so many more people employed. Maybe it is just the way they are categorizing it or how often they do it. 

```{r}
hogan_staffing <- md_senate_expenses_cleaned2 |> 
  filter(candidate == "Hogan", purpose == "STAFFING") |> 
summarise(
    total_spending = sum(amount),
    count = n())

alsobrooks_staffing <- md_senate_expenses_cleaned2 |> 
  filter(candidate == "Alsobrooks", purpose == "Salary") |> 
summarise(
    total_spending = sum(amount),
    count = n())
...
**A5** The most interesting story idea that the candidates spend a lot of money on printing. It would be intersting to research what exactly that means and where these things they are printing go. For example are they printing flyers, panflits, posters, buttons. Would be an interesting story idea to see what they are printing and why it is a top expesnse for Hogan but also a considerable one for Alsobrooks. 
