...
**A1.** LINK: https://datawrapper.dwcdn.net/XKxRO/2/  Mail-in ballots spiked in 2020 with nearly half of all bolls cast in Maryland during the 2020 election. Election day voting returned to normal levels in 2022 at around 60% with voters still using mail-in ballots. In 2022 voting by mail accounted for 19%.
...
I picked the CSV with voters aged 18-24 to see if young voters are heading to the polls. I removed all the counties and only looked at the totals by day. I learned that as the polls stay open for early voting there is a trend of more votes as the days go on. 

LINK: https://datawrapper.dwcdn.net/31vNJ/1/ 
**A2.** 
...
ev_totals_18_24 <- ev_totals_18_24 |> 
  mutate(Total = Allegany + `Anne Arundel` + `Baltimore City` + `Baltimore County` + Calvert + Caroline + Carroll + Cecil + Charles + Dorchester + Frederick + Garrett + Harford + Howard + Kent + Montgomery + `Prince George's` + `Queen Anne's` + `Saint Mary's` + Somerset + Talbot + Washington + Wicomico + Worcester) |> 
    select(Day, Total)
...
The most interesting story is that the areas with the most ActBlue contributions are right around D.C. For WinRed this is also mostly true but the donations are more spread out. It would be interesting to know more about the people who live in the northwest suburbs and why they are such strong political contributors compared to other regions of MD.

LINKS: 
WINRED: https://datawrapper.dwcdn.net/Z27e3/1/ 
ACTBLUE: https://datawrapper.dwcdn.net/WbKJy/2/ 
**A3.** 
...
acs_5yr_2022 <- load_variables(2020, "acs5", cache = TRUE)
...
md_winred_with_population <- md_winred_zip_totals |> 
  inner_join(md_population)

md_actblue_with_population <- md_actblue_zip_totals |> 
  inner_join(md_population)

md_actblue_final <- md_actblue_with_population |> 
  select(GEOID, donations, total, estimate) |>
  mutate(rate_per_1000 = (donations / estimate) * 1000)

md_winred_final <- md_winred_with_population |> 
  select(GEOID, donations, total, estimate) |>
  mutate(rate_per_1000 = (donations / estimate) * 1000)

write_csv(md_winred_final, "data/md_winred_final.csv")
write_csv(md_actblue_final, "data/md_actblue_final`.csv")
