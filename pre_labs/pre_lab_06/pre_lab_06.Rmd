...
**Task** Export the CSV file to your pre_lab_06 folder and read it into R by running the following codeblock. What problems do you see with the data? **Answer** The top line is showing is headers which is wrong becasue we removed the veritcal header. 
...
**Task** Get rid of rows we don't need by running the codeblock below. What problems remain? **Answer** Only the first column has a header. However the blank columns are now removed. 
...
**Task** Run the codeblock below. What changed, and what issues or problems remain? **Answer** Now there are separate columns for the candidate and party, and the candiates have a label for the race they were in. The data could still be sorted so more of the candidates have a label for their race and we could clean up more empty columns. 
