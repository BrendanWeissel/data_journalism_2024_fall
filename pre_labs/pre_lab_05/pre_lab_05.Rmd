...
**Task** Load the five CSV files. Do they have the same column structure? **Answer** Yes! They are not all in the same order but all the columns have the same structure. 
...
**Task** Load the Maryland 2020 county population data **Answer** Population data
...
**Task** Run the following code to join Maryland population data and the 2020 voter registration data together using the common column county as the key. How many rows are there now? How many *should* there be? **Answer** There are 24, this makes sense because for the Maryland population data set has 24. 
...
**Answer** Joining with `by = join_by(COUNTY)`
...
**Task** Now let's find the county with the highest 18+ Asian population as a percentage of all population 18 and over and also the percentage of Democrats in that county. How do these results compare to the previous task? Put your answers below. **Answer** The county with the highest percantgae is Howard and then Montgomery. Both ranked in the top 10 for black population as well. Other than those two counties all the other ones have less than 7 percent Asian population, that is only the case for 3 total counties for black population. 
...
**Task** Run the following code to load the Zip Code Tabulation Area data for Maryland. What's the datatype of the ZCTA5N? column? **Answer** DBL (number) 
