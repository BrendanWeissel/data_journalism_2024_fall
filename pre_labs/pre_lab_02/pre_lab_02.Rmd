...
**Answer** Mutate is adding a new column of data. The new column is shorting the percent of dates cast on election day. 
...
```
^turn into an actaul percentage
...
**Answer** The results are now being sorted by the highest percentage of vote on election day
...
**Answer** Yes, it shows that DEM candidates get fewer votes on election day. 
...
**Answer** str_to_lower() converts to lower case. str_to_title() converts to title case, where only the first letter of each word is capitalized.
...
**Answer** The mutate statement is sorting the data set into 5 different outputs based on the amount donated
...
**Answer** So we can refrence this new data table and go back to it if we need. We can also use this specific set of data and do calculations/filters on it.
...
**Answer:** The expenditure date column not shows YEAR-MONTH-DAY
...
**Answer** The data is showing the $ amount donated by specific month. 
