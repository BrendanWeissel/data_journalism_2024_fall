...
**Task** Run the following code and describe what the `unique` function does to the original list of words. **Answer** Makes all the same words and only has them appear once. 
...
**Task** Run the following code and describe the output of using the `unnest_tokens` function. **Answer** We not have a dataset of 677425 different words used in all the press release.m
...
**Task** Run the following code and describe what the resulting graphic shows. Is it interesting? **Answer** Eh Kind of. Most of the words are basic articles in English, such as "the" or "and" or "to." Cool that this can be done however.  
...
**Task** Run the following code and describe the results. Is it more interesting than before? **Answer** Yes. This is much more interesting because we can see what specifically he was releasing information about as we are no longer bogged down by the articles and government jargon. 
...
**Task** Run the following code and describe the results. **Answer** The top 10 words for each each are listed as well with if they are used in the release sorted by percentage
...
**Task** Run the following code and describe the results. **Answer** Now we have a datafram for the most used two word bi-grams
...
**Task** Run the following code and describe the results. **Answer** Now we have the same but for 2024, health care was second in 2022 but is the top for 2024.
...
**Task** Run the following code and describe the differences between the two results. Which one might be more useful for the press release data? **Answer** The first result has cut down the original 6 terms to 4 and then the second result cuts it down to 2. The second one is probably more useful as it is more condensed. 
...
**Task** Run the following code and describe the differences between the two results. Which one is more useful? **Answer** The one without lemmatization is more useful in my opinion becasue it shows all the specific words used. The one with lemmatization shows the themes. I personally find all the words more intersting but it depends on the desired outcome. 
...
**Task** Run the following code and describe the results. Do any of the sentiments seem incorrect or counter-intuitive? **Answer** They seem mostly right but some words don't fit in either category. Some words also need more context to fit in a category for sure. 
