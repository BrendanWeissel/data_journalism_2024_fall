...
**Task** Check for problems that occurred when loading the Texas precinct results by running the following code. How many problems were there, as shown by the number of rows in the output table showing errors? What do you think the problem is that R is describing? Answer below. **Answer** 1640 rows are showing problems. In column 12 and 13 a true or false was expected but a number is in the data
...
**Task** Run the following codeblock to glimpse the data. What data type is the "district" field? What data type is the "precinct" field? What data type is the "mail" column? Answer below. **Answer** district - number, precinct - character, mail - number
...
**Task** Run the following codeblock to group by mail votes, count the number of precinct results, then sort from highest to lowest on count. How many results are there where mail is NA? What's the implication there? Answer below. **Answer**: 402345, there was no entry for mail in those cases
...
**Task** Run the following codeblock to find the number of registered voters grouped by voter status reason, using summarise() to count. Are any of these worthy of exploration to you? Why or why not? Answer below. **Answer** I would explore the duplicate and those who requested to be removed. 
```{r}
...
**Task** Run the following codeblock to glimpse the data. What data type is the "amount" field? Answer below. **Answer** character
...
**Answer** starting with a number, using a space not an underscore and having inconsistent capitalization 
...
**Answer** It made it "x1_linenumber" so it does not start with a number to eliminate confusion. 
...
**Task** Run the following codeblock to attempt to add up the amount of all contributions. What does the error say when you run this code? What do you think the error means? Answer below. **Answer** This is the error: I think it means that sum cannot work becasue of the type of data in that section.

Error in summarise(cleaned_conowingo, total_amount = sum(amount)) : 
...
**Answer** 1 duplicate (2 rows) Derrick Hamilton
...
# display the cleaned dataset
glimpse(cleaned_conowingo) 

```

### Task 13: Clean up ZIP code

**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, standardizing the field. Look at the difference in the result - what changed? **Answer** It made every zip code on five digits. Shortening the ones that had additional numbers for more specific area codes. 
...
**Answer - 2 exist (Conowingoo and Conowing)
...
**Task** Run the following codeblock to use case_when() to fix misspellings of Conowingo in the "city" field. How many mispellings of Conowingo remain after running this code? Answer below. **Answer** 1 (Conowingoo)
...
**Task** Run the following codeblock to use case_when() to fix misspellings of Conowingo in the "city" field using both the exact match method and the str_detect() method. How many mispellings of Conowingo remain after running this code? Answer below. Add a description of what this code does to your reference notebook. **Answer** 0
